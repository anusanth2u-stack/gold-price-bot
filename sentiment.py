...
def _safe_get(url, timeout=7, headers=None):
    try:
        h = headers or {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"}
...
def _yahoo_close(ticker, range_="5d", interval="1d"):
    ...
    for base in ["https://query1.finance.yahoo.com", "https://query2.finance.yahoo.com"]:
        try:
            url = f"{base}/v8/finance/chart/{ticker}?interval={interval}&range={range_}"
            headers = {
            ...
            print(f"  ✗ Yahoo [{ticker}] via {base}: {e}")
            continue
...
# ─────────────────────────────────────────────────────────────────────────────
def get_fear_greed():
    # Try multiple CNN endpoints
    endpoints = [
        "https://production.dataviz.cnn.io/index/fearandgreed/v1",
        "https://production.dataviz.cnn.io/index/fearandgreed/v1/historical",
    ]
    for url in endpoints:
        try:
            ...
            fg = d.get("fear_and_greed") or (d.get("fear_and_greed_historical", {}).get("data") or [{}])[0]
            score = round(float(fg.get("score", fg.get("x", 50))), 1)
            label = fg.get("rating", fg.get("y", "NA"))
            ...
            print(f"  ✗ Fear&Greed [{url[:50]}]: {e}")
            continue

    # Fallback: scrape alternative source
    try:
        r = _safe_get("https://api.alternative.me/fng/")
        if r:
            d = r.json()["data"][0]
            score = int(d["value"])
...
# ─────────────────────────────────────────────────────────────────────────────
def get_nifty():
    nifty, _, nifty_chg = _yahoo_close("%5ENSEI")
    curr,  _, chg       = _yahoo_close("%5ENSEBANK")
    val  = nifty  or curr
...
def get_banking_stress():
    ...
    try:
        ...
        if stress_count >= 2:   return "STRESSED", 80
        elif stress_count == 1: return "MILD STRESS", 62
        elif stable_count >= 2: return "STABLE", 40
...
def get_geopolitical_risk():
    ...
    try:
        url = "https://news.google.com/rss/search?q=war+conflict+geopolitical+sanctions+2026&hl=en&gl=US&ceid=US:en"
        r   = _safe_get(url)
        if not r:
            return "UNKNOWN", 50
        ...
        if risk_count >= 5:     return "HIGH RISK", 85
        elif risk_count >= 2:   return "MODERATE RISK", 68
        elif peace_count >= 2:  return "LOW RISK", 38
        else:                   return "MODERATE RISK", 55
